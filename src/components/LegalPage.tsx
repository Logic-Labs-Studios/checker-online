    ...
    <div className="max-w-3xl w-full mx-auto p-6 bg-white dark:bg-stone-900 rounded-3xl shadow-xl border border-transparent dark:border-stone-800 transition-colors duration-300 mt-12 mb-20 relative">
      ...
      <div className="space-y-8">
        {sections.map((section, i) => (
          <div key={i}>
            <h2 className="text-lg font-bold text-stone-800 dark:text-stone-100 mb-2">{section.heading}</h2>
            <p className="text-stone-600 dark:text-stone-400 leading-relaxed">{section.text}</p>
          </div>
        ))}
      </div>

      <div className="mt-10 p-5 bg-stone-50 dark:bg-stone-800/50 rounded-2xl border border-stone-200 dark:border-stone-700 text-sm text-stone-500 dark:text-stone-400">
        {lang === 'pt'
          ? 'Revisão: Política adaptada para cumprimento do RGPD, CCPA e diretrizes para publicidade automatizada (Google AdSense). Damas Online não partilha informações sensíveis sobre si.'
          : 'Revision: Policy adapted for GDPR, CCPA compliance and automated advertising guidelines (Google AdSense). Checkers Online does not share sensitive information about you.'}
      </div>
    </div>
  );
}
